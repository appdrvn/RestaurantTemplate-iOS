...
- (IBAction)longPressEvent:(id)sender
{
    if (_isHandleLlongPressEvent)
        return;
    
    _isHandleLlongPressEvent = YES;
    UIActionSheet * actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                     cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
                                                     otherButtonTitles:@"保存图片", nil];
    actionSheet.tag = ACTION_SHEET_OLD_ACTIONS;
    [actionSheet showInView:self.view];
    
    _isHandleLlongPressEvent = NO;
}
...
//        self.navigationItem.rightBarButtonItem = _doneButton;
    ...
    {
        if (_removeButton)
            self.navigationItem.rightBarButtonItem = _removeButton;
        [items addObject:fixedSpace];
    }
...
{
    [self.delegate removeImageAtIndex:_currentPageIndex];
}
// KY Added - End
...
{
    if (actionSheet.tag == ACTION_SHEET_OLD_ACTIONS && buttonIndex == actionSheet.firstOtherButtonIndex)
    {
        [self savePhoto];
        return;
    }
